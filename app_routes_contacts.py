...
def contacts_list():
    ...
    # Utiliser le template standalone pour éviter les problèmes de duplication
    # Ce template n'hérite pas de base.html et définit sa propre structure HTML complète
    return render_template(
        'contacts/list_standalone.html',
        contacts=contacts,
...
def delete_contact_confirm(contact_id):
    ...
    logging.info(f"Accès à la page de confirmation de suppression pour le contact {contact_id}")

    if request.method == 'POST':
        logging.info(f"Tentative de suppression du contact {contact_id} - Méthode POST")
        confirmed = request.form.get('confirmed')
        logging.info(f"Valeur de confirmation: {confirmed}")

        if confirmed == 'yes':
            try:
                logging.info(f"Suppression des relations pour le contact {contact_id}")
                # Supprimer toutes les relations
                contact.properties = []
                contact.buildings = []

                logging.info(f"Suppression du contact {contact_id} de la base de données")
                # Supprimer le contact
                db.session.delete(contact)
                db.session.commit()

                logging.info(f"Contact {contact_id} supprimé avec succès")
                flash('Contact supprimé avec succès !', 'success')
                return redirect(url_for('contacts_list'))
            except Exception as e:
                db.session.rollback()
                logging.error(f"Erreur lors de la suppression du contact {contact_id}: {str(e)}")
                flash(f'Erreur lors de la suppression du contact : {str(e)}', 'danger')
                return redirect(url_for('contact_detail', contact_id=contact.id))

    # Si c'est une requête GET ou si la confirmation n'a pas été donnée
    logging.info(f"Affichage de la page de confirmation pour le contact {contact_id}")
    return render_template('contacts/delete_standalone.html', contact=contact)
